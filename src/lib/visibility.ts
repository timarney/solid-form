...
import { GroupsType, FormValues } from "./types";
...
export const getVisibleGroupsBasedOnValuesRecursive = (
  ...
  if (typeof nextAction === "string") {
    ...
    const catchAllRule = nextAction.find((action) =>
    ...
    for (const action of nextAction) {
      ...
      if (
        valuesWithMatchedIds[elementId] &&
        valuesWithMatchedIds[elementId] === action.choiceId
      ) {
        matchFound = true;
        return getVisibleGroupsBasedOnValuesRecursive(
          formRecord,
...
export const checkPageVisibility = (
  ...
  if (
    !formRecord.form.groups ||
    Object.keys(formRecord.form.groups).length === 0
  ) {
    return true;
  }
  ...
  return !!visibleGroups.find(
    (visitedGroupId: string | undefined) => visitedGroupId === groupId
  );
};
