 ...
 * @returns
 */
export const isChoiceInputType = (type: string) =>
...
export const mapIdsToValues = (
  ...
  const valueIds = elementIds.filter(
    (id) => values[id] && values[id].length > 0
  );
 ...
 */
export const getValuesWithMatchedIds = (
  formElements: FormElement[],
  values: FormValues
) => {
    ...
    if (isChoiceInputType(el.type) && choices && Array.isArray(choices)) {
      if (Array.isArray(value)) {
        ...
        const choiceIndex = choices.findIndex(
          (choice) =>
            (choice.en !== "" && choice.en === value) ||
            (choice.fr !== "" && choice.fr === value)
        );
        if (choiceIndex > -1) {
          newValues[key] = `${el.id}.${choiceIndex}`;
        } else {
...
export const findGroupByElementId = (
  ...
  return Object.keys(groups).find((groupKey) =>
    inGroup(groupKey, elementId, groups)
  );
};
