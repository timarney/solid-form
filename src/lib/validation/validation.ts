...
export const isFieldResponseValid = (
  ...
  switch (componentType) {
    case FormElementTypes.textField: {
      ...
      if (validator.required && !typedValue)
    ...
    case FormElementTypes.checkbox: {
      if (validator.required) {
        if (
          ...
          (value === undefined ||
            !Array.isArray(value) ||
            (value as Array<string>).length !=
              formElement.properties.choices?.length)
        ) {
    ...
    case FormElementTypes.fileInput: {
      ...
      if (
        fileInputResponse.name &&
        !isFileExtensionValid(fileInputResponse.name)
      )
      ...
      if (
        fileInputResponse.size &&
        !isIndividualFileSizeValid(fileInputResponse.size)
      ) {
    ...
    case FormElementTypes.dynamicRow: {
      ...
      const groupErrors = (value as Array<Responses>).map((row) => {
        ...
        (formElement.properties.subElements || []).forEach(
          (subElement, index) => {
